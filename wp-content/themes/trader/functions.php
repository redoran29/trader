...
add_action('wp_ajax_loadmore', 'my_action_callback');
add_action('wp_ajax_nopriv_loadmore', 'my_action_callback');

function my_action_callback(){
	global $wpdb;
	$id = $_POST['id'];
    $query = $wpdb->get_results( "SELECT * FROM wp_cf7_data_entry WHERE data_id = $id" );
    //print_r($query);
    $array = array();
	foreach ($query AS $qb){
    	$name = $qb->name;
        ...
        if($name == 'geographic-region'){
            $gr = $qb->value;
            array_push($array, $gr);
        }
        if($name == 'street-address-1'){
            $street1 = $qb->value;
            array_push($array, $street1);
        }
        if($name == 'street-address-2'){
            $street2 = $qb->value;
            array_push($array, $street2);
        }
	}
    $post_items[] = 'oid=<00D1r000000d8Xs>';
    $post_items[] = 'school_name=' . $school;
    $post_items[] = 'geographic-region=' . $gr;
    $post_items[] = 'street-address-1=' . $street1;
    $post_items[] = 'street-address-2=' . $street2;
    $post_items[] = '00D1r000000d8Xs=';
    echo $post_string = implode ('&', $post_items);
    ...
    echo curl_setopt($ch, CURLOPT_URL, $con_url);
    // Set the method to POST
    echo curl_setopt($ch, CURLOPT_POST, 1);
    // Pass POST data
    echo curl_setopt( $ch, CURLOPT_POSTFIELDS, $post_string);
    echo curl_exec($ch); // Post to Salesforce
    curl_close($ch); // close cURL resource

    /*$body = "произошли изменения: <br>";

    $mail_subject = "сводка";
    $to = "d.volchek@comradeweb.com";
...
require_once ('blocks/trader.php');
//require_once ('blocks/db.php');

function pre_print($x) {
    echo '<pre>';
    print_r($x);
    echo '</pre>';
}